...
<main>
  <section class="text-center pt-14">
    ...
    <h3 class="text-xl text-white pb animate-jump-in animate-delay-[1000ms]">Here are your top 50 artists based on your all-time listening habits.</h3>
  </section>
  <section>
          ...
          <div class="skills w-full bg-green-500 text-left p-4 rounded-xl">
            ...
            <ul class="list-none p-0 m-0">
              {#each convertGenresToLi(artist.genres) as genre}
                <li class="inline-block border border-gray-800 rounded-sm text-xs m-1 py-1 px-2">{genre}</li>
              {/each}
            </ul>
          </div>
        </div>
